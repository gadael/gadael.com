...
.factory('hexoLocation', ['$window', 'hexoPage', function($window, hexoPage) {
    return function(url, skipUpdate) {
        ...
        if (!$window.history) {
            ...
            var path = index > 0 ? prepareAddressUrl(url.substring(0, index)) : url;
    ...
    function prepareAddressUrl(url) {
        ...
        } else if (url && url.length && !/\.html/.test(url) && url.indexOf('#') == -1 && url.indexOf('?') == -1 && url.charAt(url.length-1) != '/') {
            url += '/';
        }
        return url;
    }
...
function($http,   $document,   prepareAjaxUrl,   prepareAjaxContent,   hexoTimestamp,   $q) {
    ...
    return function(url) {
        var production = angular.element($document[0].body).data('plainview') == 'on';
